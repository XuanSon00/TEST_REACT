...
export default function Tablet() {
    const tabletProducts = products.filter(product => product.tag.includes('tablet devices'));
    const [modal, setModal] = useState(false);
    const toggleModal = () => {
      setModal(!modal);
    };
    const productItems = tabletProducts.map(product => (
    ...
    const handleProductClick = (productId) =>{
        const product = tabletProducts.find(item => item.id === productId);
