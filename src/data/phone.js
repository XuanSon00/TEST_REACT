...
export default function Phone() {
  const phoneProducts = products.filter(product => product.tag.includes('samsung-galaxy') || product.tag.includes('iphone'));
  const [modal, setModal] = useState(false);
  const toggleModal = () => {
    setModal(!modal);
  };
    const productItems = phoneProducts.map(product => (
    ...
    const handleProductClick = (productId) =>{
        const product = phoneProducts.find(item => item.id === productId);
