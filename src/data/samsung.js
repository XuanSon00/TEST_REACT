...
export default function Samsung() {
    const samsungProducts = products.filter(product => product.tag.includes('samsung'));
    const [modal, setModal] = useState(false);
    const toggleModal = () => {
      setModal(!modal);
    };
    const productItems = samsungProducts.map(product => (
    ...
    const handleProductClick = (productId) =>{
        const product = samsungProducts.find(item => item.id === productId);
